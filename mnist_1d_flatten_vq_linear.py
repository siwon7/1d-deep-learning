...
train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=16, shuffle=True) #batch 32
...
test_loader = torch.utils.data.DataLoader(dataset=test_dataset, batch_size=16, shuffle=False) #batch 32
...
if choice == "Y":
    ...
    optimizer = optim.Adam(vqvae.parameters(), lr=4.5e-4) #4.5e-4 1e-3
    num_epochs = 20
...
rnn_model_path = 'model/VQ_Linear_{num_embedding}.pth'

rnn_loaded = False
if os.path.exists(rnn_model_path):
    choice = input("학습된 VQ_Linear 모델이 있습니다. 로드하시겠습니까? (Y/N): ").strip().upper()
else:
    choice = "N"
...
if choice == "Y":
    ...
    rnn_loaded = True
    print("VQ_Linear 모델이 로드되었습니다.")
else:
    optimizer = optim.Adam(classifier.parameters(), lr=4.5e-4) #lr=4.5e-4 lr=1e-3
    ...
    for epoch in range(num_epochs):
        ...
        print(f'Epoch [{epoch+1}/{num_epochs}], Linear Regression Loss: {epoch_loss/len(train_loader):.4f}')
...
all_labels = []
all_predictions = []
# VQ_Linear 폴더 생성 및 하위 폴더 생성
base_dir = f'VQ_Linear_{num_embeddings}'
...
os.makedirs('VQ_Linear_graphs', exist_ok=True)
...
test_time_df.to_csv('VQ_Linear_graphs/test_time.csv', index=False)
...
predictions_df.to_csv('VQ_Linear_graphs/predictions.csv', index=False)
...
if not vqvae_loaded:
    ...
    plt.savefig(os.path.join('VQ_Linear_graphs', 'vqvae_loss.png'))
    plt.close()
...
if not rnn_loaded:
    ...
    plt.figure()
    epochs = range(1, num_epochs + 1)
    plt.plot(epochs, rnn_losses, label='Linear Regression Loss', color='blue')
    plt.xlabel('Epochs')
    plt.ylabel('Linear Regression Loss', color='blue')
    plt.tick_params(axis='y', labelcolor='blue')
    plt.title('Linear Regression Training Loss')
    plt.legend(loc='upper right')
    plt.savefig(os.path.join('VQ_Linear_graphs', 'linear_loss.png'))
    plt.close()
    ...
    plt.title('Linear Regression Test Accuracy')
    plt.legend(loc='upper right')
    plt.savefig(os.path.join('VQ_Linear_graphs', 'linear_accuracy.png'))
    plt.close()
    ...
    results_df = pd.DataFrame({
        ...
        'Linear Regression Loss': rnn_losses,
        'Accuracy': test_accuracies
    })

    results_df.to_csv(os.path.join('VQ_Linear_graphs', 'loss_accuracy.csv'), index=False)
